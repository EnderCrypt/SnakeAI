 ...
 */
public class ParanoidImprovedEatingAI implements AI
	{
	@Override
		...
		if (snake.directionBlocked(snake.direction))
			...
			if (!Main.game.map[nextPos.x][nextPos.y].equals(MapObject.FOOD)) // dont check for food on the sides if theres already food just infront of you
				...
				for (int i=0;i<20;i++)
					...
					{
					boolean canMoveLeft = snake.directionBlocked(snake.direction.getLeft());
					boolean canMoveRight = snake.directionBlocked(snake.direction.getRight());
